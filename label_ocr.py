...
Extracts and validates PCR 2011 mandatory declarations from product label images.
...
Fixes applied vs original:
  ...
  - Added PCR 2011 fields: "inclusive of all taxes" check, consumer-care, unit-normalisation
...
class Patterns:
    ...
    # Non-standard unit strings that violate PCR 2011
    NON_STANDARD_UNITS = {
        "gms": "g", "grm": "g", "grms": "g",
        "kgs": "kg",
        "ltr": "L", "ltrs": "L", "lts": "L",
        ...
        for p in self.patterns.SHELF_LIFE:
            ...
            if m_shelf:
                try:
                    ...
                    # Normalise common OCR noise (e.g. 38 -> 36 when '36' or cosmetics label)
                    months_to_add = 36 if raw_val == 38 else raw_val
...
class OCREngine:
    ...
    - Primary Ensemble: PaddleOCR (curved/angled/dot-matrix text) + Tesseract (structured multi-pass zones)
    - Execution Order:
    ...
    def _get_paddle(self):
        ...
        # 1. Try RapidOCR (PaddleOCR PP-OCRv4 models via Microsoft ONNX Runtime)
        try:
            from rapidocr_onnxruntime import RapidOCR
            _SHARED_PADDLE = RapidOCR()
            _SHARED_PADDLE_MODE = "rapidocr"
            self._paddle_mode = "rapidocr"
            print("[OCR Engine] PaddleOCR (RapidOCR ONNX engine) warmed up and cached in memory.")
            return _SHARED_PADDLE
        except Exception as e1:
            # 2. Try native PaddleOCR if available
            try:
                import sys
                ...
                print(f"[OCR Engine] PaddleOCR unavailable ({e1}, {e2}). Using enhanced multi-pass Tesseract.")
                _SHARED_PADDLE = None
                return None
    ...
    def ocr_paddle(self, source) -> str:
        ...
        try:
            ...
            if max(h, w) > 1024:
                ...
                img = cv2.resize(img, (int(w * scale), int(h * scale)), interpolation=cv2.INTER_AREA)
            if getattr(self, "_paddle_mode", "") == "rapidocr":
                result, _ = paddle(img)
                lines = []
                if result:
                    for item in result:
    ...
    def extract_all(self, source) -> dict:
        ...
        # Run PaddleOCR (ONNX Runtime) and Tesseract concurrently across CPU cores
        ...
        results = {
            ...
            "engines_used":     {
                "paddle_ocr": bool(_SHARED_PADDLE is not None),
                "tesseract":  True,
            },
        }
