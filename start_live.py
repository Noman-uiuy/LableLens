...
def main() -> None:
    ...
    try:
        ...
        print("[Tunnel] Requesting a temporary HTTPS URL…", flush=True)
        tunnel_log_path = BASE_DIR / "tunnel.log"
            ...
            for _ in range(30):
                ...
                if tunnel_log_path.exists():
                    ...
                    if match:
                        ...
                        (BASE_DIR / "tunnel_url.txt").write_text(tunnel_url.strip(), encoding="utf-8")
                        print("\n" + "=" * 64, flush=True)
                        print("  TEMPORARY PUBLIC DEMO URL", flush=True)
                        print(f"  {tunnel_url}", flush=True)
                        print("=" * 64 + "\n", flush=True)
                        break
            if not tunnel_url:
                print("[Tunnel] Failed to obtain URL within 30 seconds. Check tunnel.log", flush=True)
            tunnel.wait()
        finally:
